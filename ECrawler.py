...
try:
    ...
    try:
        ...
        print('%s ECrawler runs only on Python 3.2 and above.' % info)
        quit()
    ...
    parser.add_argument('--keys', help='find secret keys', dest='api',
                        action='store_true')
    parser.add_argument('--update', help='update ECrawler', dest='update',
                        action='store_true')
    ...
    failed = set()  # URLs that ECrawler failed to crawl
    ...
    print(Fore.RED + "\nThanks using ECrawler" + Fore.WHITE)